...
const _BUNDLE_ID = 'com.expo.shareextension';
...
async function runShareExtensionTests() {
  ...
  tests.tests.push(
    await runner.runTest(
      'Share extension has proper entitlements',
      async () => {
        const validator = buildRunner.getValidator();
  ...
  tests.tests.push(
    await runner.runTest('Share extension Info.plist configured', async () => {
      const _iosPath = path.join(SHARE_APP, 'ios');
      const validator = buildRunner.getValidator();
      const result = await validator.validateTarget(
        SHARE_APP,
        'share',
  ...
  tests.tests.push(
    await runner.runTest(
      'App Groups match between main app and extension',
      async () => {
        const validator = buildRunner.getValidator();
