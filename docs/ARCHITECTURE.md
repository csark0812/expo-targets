...
## Reference-in-Place Architecture

expo-targets uses a **reference-in-place** architecture where native source files remain in `targets/` and are referenced directly by Xcode, rather than being copied to `ios/`.

### Core Concept

**Traditional Approach (Pre-1.0)**:

```
targets/my-widget/ios/Widget.swift  [SOURCE]
           ↓ COPY
ios/MyWidgetTarget/Widget.swift     [COPY - managed by prebuild]
```

**Reference-in-Place (1.0+)**:

```
targets/my-widget/ios/Widget.swift  [SOURCE - single copy]
           ↑ REFERENCE
Xcode Project (expo:targets group)  [REFERENCE - points to original]
```
...
Swift/Kotlin source files stay in `targets/TARGETNAME/ios/` or `targets/TARGETNAME/android/`:

```
targets/weather-widget/
├── expo-target.config.json
├── index.ts                    [TypeScript runtime code]
└── ios/
    ├── Widget.swift            [User Swift source - stays here]
    ├── Provider.swift          [User Swift source - stays here]
    └── WidgetView.swift        [User Swift source - stays here]
```

**Benefits:**

- Edit in Xcode or VS Code - same file
- Git tracks only source, not copies
- No sync issues between locations

#### Generated Metadata Files

Build artifacts generated by expo-targets go in `targets/TARGETNAME/ios/build/`:

```
targets/weather-widget/
└── ios/
    ├── Widget.swift            [User source]
    └── build/                  [Generated - git ignored]
        ├── Info.plist          [Generated from config]
        ├── Assets.xcassets/    [Generated colors & assets]
        │   ├── AccentColor.colorset/
        │   └── BackgroundColor.colorset/
        ├── generated.entitlements          [Generated entitlements]
        └── ReactNativeViewController.swift [Generated if using RN]
```

**Benefits:**

- Clear separation: user code vs generated artifacts
- Generated files git-ignored
- Regenerated on each prebuild/sync

### Xcode Project Integration

#### Virtual expo:targets Group

Xcode organizes referenced files in a virtual `expo:targets` group:
...
├── YourAppTests/
├── Pods/
└── expo:targets/                    [Virtual group - not a real directory]
    ├── WeatherWidget/               [Target subgroup]
    │   ├── Widget.swift             [→ ../../targets/weather-widget/ios/Widget.swift]
    │   ├── Provider.swift           [→ ../../targets/weather-widget/ios/Provider.swift]
    │   ├── Info.plist               [→ ../../targets/weather-widget/ios/build/Info.plist]
    │   └── Assets.xcassets          [→ ../../targets/weather-widget/ios/build/Assets.xcassets]
    └── MyClip/                      [Another target]
        └── ...
```

**Virtual Group** means:

- Files appear organized in Xcode navigator
- Actual files remain in `targets/` on disk
- References use relative paths from ios/ directory
- Xcode treats them like project files

#### File References

expo-targets adds `PBXFileReference` entries pointing to external paths:

```xml
<!-- In project.pbxproj -->
<PBXFileReference>
  <isa>PBXFileReference</isa>
  <lastKnownFileType>sourcecode.swift</lastKnownFileType>
  <name>"Widget.swift"</name>
  <path>"../../targets/weather-widget/ios/Widget.swift"</path>
...
- Relative to `ios/` directory
- `../../targets/` goes up to project root, then into targets/
- Works in both Xcode and build system

#### Build Settings

Generated files referenced via build settings:

```
INFOPLIST_FILE = "../../targets/weather-widget/ios/build/Info.plist"
CODE_SIGN_ENTITLEMENTS = "../../targets/weather-widget/ios/build/generated.entitlements"
```

### Implementation Details

#### Plugin Flow (withXcodeChanges.ts)
...
3. **Create Virtual Group**
   ...
   ```typescript
   const virtualGroupUuid = Xcode.ensureExpoTargetsGroup({ project });
...
4. **Add File References**
   ...
   const fileRefUuid = Xcode.addExternalFileReference({
     ...
     filePath: relativePath, // ../../targets/widget/ios/Widget.swift
     fileName: file,
   });
   ```

5. **Add to Build Phases**
   ```typescript
...
New path helpers in `packages/expo-targets/plugin/src/ios/utils/paths.ts`:

```typescript
// Build directory
getTargetBuildPath({ projectRoot, targetDirectory });
// → /path/to/project/targets/widget/ios/build

// Generated files
getTargetInfoPlistPath({ projectRoot, targetDirectory });
// → /path/to/project/targets/widget/ios/build/Info.plist

getTargetEntitlementsPath({ projectRoot, targetDirectory });
// → /path/to/project/targets/widget/ios/build/generated.entitlements

getTargetAssetsPath({ projectRoot, targetDirectory, isStickers });
// → /path/to/project/targets/widget/ios/build/Assets.xcassets

// User source files
getTargetSourcePath({ projectRoot, targetDirectory, fileName });
// → /path/to/project/targets/widget/ios/Widget.swift
```

### Comparison: Copy vs Reference

| Aspect              | Copy (Pre-1.0)         | Reference (1.0+)                |
| ------------------- | ---------------------- | ------------------------------- |
| **Source Location** | `targets/`             | `targets/`                      |
| **Build Location**  | `ios/TargetName/`      | `targets/TARGETNAME/ios/build/` |
| **Xcode Shows**     | `ios/TargetName/`      | `expo:targets/TargetName/`      |
| **Edit in Xcode**   | Changes in `ios/`      | Changes in `targets/`           |
| **Edit in VS Code** | Changes in `targets/`  | Changes in `targets/`           |
| **Sync Required**   | Yes (copy on prebuild) | No (same file)                  |
| **Git Tracking**    | Both locations         | Only `targets/`                 |
| **Bare RN Support** | No                     | Yes (sync CLI)                  |
...
1. Plugin discovers targets in `targets/*/expo-target.config.json`
2. Generates `ios/` directory from scratch
3. Creates virtual `expo:targets` group
4. Adds file references to `targets/` (no copying)
5. Generates metadata in `targets/TARGETNAME/ios/build/`
6. Updates Xcode project and Podfile

#### Bare React Native (Sync CLI)

```bash
npx expo-targets sync
```

1. Reads existing `ios/` directory
2. Discovers targets in `targets/*/expo-target.config.json`
3. Updates Xcode project (same as plugin)
4. Preserves manual Xcode changes
5. User runs `pod install` manually

### Benefits

1. **Single Source of Truth**
   - One copy of each Swift file
   - Eliminates sync issues
   - Clear ownership

2. **Developer Experience**
   - Edit anywhere (Xcode or VS Code)
   - Changes immediately reflected
   - No waiting for copy operations

3. **Bare RN Compatibility**
   - Sync CLI updates existing projects
   - No need for full CNG workflow
   - Manual Xcode control retained
...
5. **Build Performance**
   ...
   - Faster prebuild/sync
   - Incremental builds work better

### Limitations

1. **Xcode Shows External Files**
   - Files appear outside normal project structure
   - Can be confusing initially
   - Mitigated by virtual group organization

2. **Path Complexity**
   - Relative paths can be fragile
   - Must handle path resolution correctly
   - Breaking changes if structure changes

3. **Generated Files Scattered**
...
For projects upgrading from pre-1.0:

1. Delete old `ios/TargetName/` directories
2. Update expo-targets package
