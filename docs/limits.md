...
| `feature` | Surface that did not green |
| `home` | `limits.md` \| later PR \| `wont-do` |
| `spike` | Path under `artifacts/spikes/` |
| `owner` | PR author |

**D1 done** when this policy and register section exist and every open leftover for the shipping PR has a row. **D1-scaffold** (stubs only) is not D1 done.
...
Deepen already-green ids before you add shallow types: share/action(+native); safari(+native)/content-blocker; widgets/keyboard/photo-editing/clip(+native)/stickers. **file-provider** deepens via open→App Group `fp:*` (seed *UI* still empty on Air). **app-intent** keeps Shortcuts list floor (Unable to run App Shortcut on Air — see spike). Exit per id = required **P** surfaces green; S3a rows only for proven non-P features.
