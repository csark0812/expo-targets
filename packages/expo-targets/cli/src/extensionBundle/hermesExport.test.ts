...
function fakeHermesRunner(opts?: {
  bundleBytes?: Buffer;
  withAssets?: boolean;
}) {
  return (_cmd: string, args: string[]) => {
    ...
    if (out) {
      ...
      fs.writeFileSync(
        out,
        opts?.bundleBytes ?? Buffer.from('// hermes fake\n')
      );
    }
    if (opts?.withAssets && assetsDest) {
...
function seedShareProject(project: string): void {
  fs.writeFileSync(
  ...
  try {
    ...
    exportTargetHermesBundle({
      ...
      run: (_cmd, args) => {
        ...
        return fakeHermesRunner()(_cmd, args);
      },
    });
...
test('publish layout drops Metro --assets-dest files (OTA image gap)', () => {
  ...
  try {
    ...
    const { code } = runExportExtensionBundles({
      ...
      assetsRoot: assets,
      hermes: true,
      runHermes: fakeHermesRunner({ withAssets: true }),
    });
    expect(code).toBe(0);
