...
Thin expo-targets example host for `device-activity-monitor`.
...
# From repo root
bun install
cd examples/device-activity-monitor
...
```bash
bun run examples:devicewright:matrix --ids=device-activity-monitor
```
